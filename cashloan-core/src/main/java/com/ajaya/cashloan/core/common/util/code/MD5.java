package com.ajaya.cashloan.core.common.util.code;
...
public class MD5 {
    ...
    /**
     * 下面这些 S11-S44 实际上是一个 4*4 的矩阵，在原始的 C 实现中是用#define 实现的， 这里把它们实现成为 static
     * final 是表示了只读，切能在同一个进程空间内的多个 Instance 间共享
     */
     ...
     * 加的盐
     */
    private static final String SALT = "SPJkzmLU8QtfBM7xBcgCq8vd6lYuQ8Vu";

    /*
     * digestHexStr 是 MD5 的唯一一个公共成员，是最新一次计算结果的 　 16 进制 ASCII 表示.
     */
    private static String digestHexStr;

    /*
     * 下面的三个成员是 MD5 计算过程中用到的 3 个核心数据，在原始的 C 实现中 被定义到 MD5_CTX 结构中
     */
     ...
     * 简单的位运算，可能出于效率的考虑把它们实现成了宏，在 java 中，我们把它们 　　实现成了 private 方法，名字保持了原来 C 中的。
     */
    ...
    /*
     * FF,GG,HH 和 II 将调用 F,G,H,I 进行近一步变换 FF, GG, HH, and II transformations for
    ...
    /*
     * Encode 把 long 数组按顺序拆成 byte 数组，因为 java 的 long 类型是 64bit 的， 只拆低 32bit，以适应原始 C 实现的用途
     */
     ...
     * 　因为 java 中的 byte 的 toString 无法实现这一点，我们又没有 C 语言中的 sprintf(outbuf,"%02X",ib)
     */
    ...
    public static synchronized String encode(String inbuf) {
        md5Init();
     ...
     */
    public static synchronized String getMD5ofStr(String inbuf) {
        md5Init();
        md5Update(inbuf.getBytes(), inbuf.length());
        md5Final();
        digestHexStr = "";
    ...
    /**
     * 加盐的 md5 值。先对字符串进行 MD5 进行加密，然后将加密后的字符串加上盐再进行一次 MD5 加密
     *
     * @param inbuf 需做 md5 的字符串
     * @return
     */
    public static String encodeByMd5ThenSalt(String inbuf) {
        return getMD5ofStr(getMD5ofStr(inbuf) + SALT);
    }
     ...
     * @return
     */
    public static String encodeByMd5ThenSalt(String inbuf, String salt) {
        return getMD5ofStr(getMD5ofStr(inbuf) + salt);
    }
