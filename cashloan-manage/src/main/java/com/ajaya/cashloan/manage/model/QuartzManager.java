...
public class QuartzManager {
	...
	public static void addJob(String jobName, String jobGroupName,
		...
		try {
			Scheduler sched = gSchedulerFactory.getScheduler();
			JobDetail jobDetail = new JobDetailImpl(jobName, JOB_GROUP_NAME, jobClass);// 任务名，任务组，任务执行类
			// 触发器
			CronTriggerImpl trigger = new CronTriggerImpl(jobName, TRIGGER_GROUP_NAME);// 触发器名,触发器组
			trigger.setCronExpression(time);// 触发器时间设定
			sched.scheduleJob(jobDetail, trigger);
	...
	public static void modifyJobTime(String triggerName,
		...
		try {
			...
			/*CronTrigger trigger = (CronTrigger) sched.getTrigger(triggerName,
					triggerGroupName);*/
			CronTriggerImpl trigger = (CronTriggerImpl) sched.getTrigger(new TriggerKey(triggerName,
					triggerGroupName));
			if (trigger == null) {
				return;
			}
