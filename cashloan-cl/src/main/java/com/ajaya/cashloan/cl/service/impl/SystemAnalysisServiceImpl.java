...
import com.ajaya.cashloan.cl.mapper.SystemAnalysisMapper;
	...
	public List<RepayAnalisisModel> dayRepayAnalisis(Map<String, Object> params) {
		...
		try {
			...
			for (int i = 0; i < dateList.size(); i++) {
				...
				String repayCount = null;
				if(params.get("channelIds") != null){
					map.put("channelIds", params.get("channelIds"));
					//说明是渠道管理人员
					repayCount = systemAnalysisMapper.repayCountForChannel(map);
				} else {
					repayCount = systemAnalysisMapper.repayCount(map);
				}
				repayModel.setRepayCount(repayCount==null?"0":repayCount);
				...
				if(params.get("channelIds") != null){
					map.put("channelIds", params.get("channelIds"));
					//说明是渠道管理人员
					overdueCount = systemAnalysisMapper.overdueCountForChannel(map);
				} else {
					overdueCount = systemAnalysisMapper.overdueCount(map);
				}
				repayModel.setOverdueCount(overdueCount==null?"0":overdueCount);
				...
				if(params.get("channelIds") != null){
					map.put("channelIds", params.get("channelIds"));
					//说明是渠道管理人员
					repayAmt = systemAnalysisMapper.repayAmtForChannel(map);
				} else {
					repayAmt = systemAnalysisMapper.repayAmt(map);
				}
				repayModel.setRepayAmt(repayAmt==null?"0":repayAmt);
				
				
				String penaltyRepayAmt = null;
				if(params.get("channelIds") != null){
					map.put("channelIds", params.get("channelIds"));
					//说明是渠道管理人员
					penaltyRepayAmt = systemAnalysisMapper.penaltyRepayAmtForChannel(map);
				} else {
					penaltyRepayAmt = systemAnalysisMapper.penaltyRepayAmt(map);
				}
				repayModel.setPenaltyRepayAmt(penaltyRepayAmt==null?"0":penaltyRepayAmt);
